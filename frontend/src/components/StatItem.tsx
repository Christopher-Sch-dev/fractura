...
export function StatItem({ stat, i }: StatItemProps) {
  ...
  return (
    <div
      ...
      onClick={scramble}
      className="flex gap-4 group cursor-pointer items-center p-3 hover:bg-[rgba(0,229,255,0.04)] border border-transparent hover:border-[var(--color-primary-20)] transition-all duration-700 relative overflow-hidden"
    >
      <div className="w-8 h-8 bg-[var(--bg-deep)] border border-[var(--border-dim)] flex items-center justify-center text-[var(--text-muted)] group-hover:text-[var(--color-primary)] group-hover:border-[var(--color-primary-40)] transition-all duration-700 group-hover:scale-110">
        <StatIcon index={i} />
      </div>
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <p className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-[0.35em] mt-0.5 group-hover:text-[var(--color-primary)] transition-colors system-heartbeat">
          {stat.sub}
        </p>
        {stat.detail && (
...
function StatIcon({ index }: { index: number }) {
  const icons = ['$', '⬡', '⌂', '◈']
  return (
    <span className="text-sm font-black text-[var(--text-muted)] group-hover:text-[var(--color-primary)] transition-colors">
      {icons[index] ?? '◉'}
    </span>
  )
}